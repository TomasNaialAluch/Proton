...
export default function PlayerQueueDrawer({ open, onClose }: PlayerQueueDrawerProps) {
  ...
  // Portal renders at document.body to escape any CSS transform/stacking context
  // (e.g. the scale-125 wrapper around fullscreen controls creates a containing block
  // that breaks position:fixed on descendants).
  const drawer = isFullscreen ? (
    <div
      role="dialog"
      aria-label="Queue"
      className="fixed top-14 right-4 z-[200] flex w-80 flex-col overflow-hidden rounded-2xl border border-[var(--color-border)] shadow-2xl"
      style={{ background: "var(--color-surface)", maxHeight: "calc(100vh - 80px)" }}
    >
      {header}
      {list}
    </div>
