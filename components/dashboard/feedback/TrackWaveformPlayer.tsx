...
/**
 * Just playback + waveform — no title/genre/BPM/key header. Split out of
 * what used to be `FeedbackTrackPlayer.tsx` so this can be embedded
 * somewhere that already shows that metadata elsewhere on the page (Track
 * Detail's own header), without repeating it. See
 * docs/feature-discover-producers.md, "Scoring is global, not Discover-only."
 */
export default function TrackWaveformPlayer({ track }: { track: Track }) {
  ...
  /** Playing is page-scoped: leaving stops it and resumes the global player. */
  useEffect(() => {
    return () => {
