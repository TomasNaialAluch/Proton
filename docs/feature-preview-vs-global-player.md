...
### 5.2 No purchase CTA — confirmed, and why

Proton Radio itself doesn't sell the file — it's a radio/DJ-pool platform. The real "GET FULL TRACK" button is a hand-off to buy legally elsewhere (Beatport or wherever that release is distributed) — giving away a copyrighted master for free, or serving it as a "full download" from Proton itself, would be actual piracy, not a product decision. That's specific to Proton Radio's real business (DJs buying tracks for sets).

**Our product has no equivalent transaction.** Our users are producers/label managers evaluating tracks (feedback, remix requests, A&R), not DJs buying music. So the bar's right-most slot in our version isn't "no CTA yet, pending Beatport integration" — it's **permanently not a thing we build**, because there's no purchase to hand off to in this product's story. The **close ✕** takes that visual position instead (see section 4.3 — it's the control that triggers the resume-the-show decision).
